...
| filter | [过滤搜索函数](/form/repeater/config#搜索) | (v) => array[any] | noop |
| renderOper | [自定义操作按钮](/form/repeater/config#自定义操作按钮渲染) | (btnList, core) => JSX | noop |
| dialogConfig | [自定义弹窗表单内容](/form/repeater/config#自定义tablerepeater的弹窗表单内容) | {} | {} |

## Repeater下的FormItem JSX 属性

| 属性名 | 作用 | 类型 | 默认值 |
| ------ | ------ | ------ | ----- |
| renderCell | [自定义展示字段](/form/repeater/config#自定义展示字段) | (value, payload: { values, id, core, index } ) => JSX | noop |
