...
在NoForm的定义中，表单数据分为 [**值(values)**](/core/value)，[**状态(status)**](/core/index#获取修改错误信息error)，[**错误(errors)**](/core/errors)， [**属性(props)**](/core/props)， 同时它提供了一系列的API去控制这些数据。
...
| validateConfig | 校验规则 | 参考[校验](/core/validate)一节， 规则与[async-validator](https://github.com/yiminghe/async-validator)一致 | {} |
...
| interceptor | 拦截器 | 参考[拦截器](/core/interceptor)一节 | {} |
