...
| defaultFilterValues | 初始化请求数据 | Object | null |
| defaultPage | 初始化页数 | number | 1 |
| autoLoad | 是否初始化完成立即加载请求 | boolean | true |
| params | 页面参数信息 | Object | null |
| filterConfig | 同[FormCore构建参数](/core/index#构建参数) | Object | null |

## Filter 属性

| 参数名 | 作用 | 类型 | 默认值 |
| ------ | ------ | ------ | ----- |
| noDefaultLayout | 取消默认布局 | boolean | false |
| cols | 布局列数 | number | 3 |
| onSubmit | 点击提交时的回调 | function | noop |
| onReset | 点击清空时的回调 | function | noop |
...
| listenFilterChange | 是否监听搜索区域更新 | boolean | false |
