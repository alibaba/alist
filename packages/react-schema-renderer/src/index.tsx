...
import { registerListFuncs, registerListComponent,
  cleanListComponentRegistry, cleanListFunctionRegistry,
  getComponentsRegistry, getFuncsRegistry
} from './shared'
...
const renderSchema = (props, contextProps) => {
  const { funcRegistry, componentsRegistry } = contextProps;
  ...
  if (!(_.has(componentsRegistry, componentName))) { // 不存在
...
const SchemaSolver = (props) => {
  ...
  return <ListDomainContext.Provider value={context}>
    {(componentsTree || []).map((componentSchema) => {
      const { component, props: rootProps, children: rootChilds } = renderSchema(componentSchema, context);      
